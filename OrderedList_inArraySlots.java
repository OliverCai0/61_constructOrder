...
public class OrderedList_inArraySlots
    ...
    public OrderedList_inArraySlots
        ...
        list_iAS = new ArrayList<Integer>(unordered.size());
     ...
     */
     private int champIndex( ArrayList<Integer> challengers) {
        int index = 0;
        int champ = challengers.get(0);
        for(int count = 0; count < challengers.size(); count ++){
          if(challengers.get(count) < champ){
            index = count;
            champ = challengers.get(count);
          }
        }
        return index; 
     }
